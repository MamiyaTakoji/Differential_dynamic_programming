classdef DDP%真的是iLQG吗？
    properties
        CostFun
        StateFun
    end
    methods
        function [xhat,uhat] = NStepDDP(obj,x,u,N)
            xhat = x;uhat = u;
            for i = 1:N
                [xhat,uhat] = obj.OneStepDDP(xhat,uhat);
            end
        end
        function [xhat,uhat] = OneStepDDP(obj,x,u)
            %u和x为已知的控制和轨迹
            %返回的是修正后的轨迹
            %轨迹的长度应该是控制量长度+1
            N = length(x);
            ...
            alpha = 1;
            IsBreak = false;
            for count = 1:20%差不多得了，别把电脑搞死循环了
                for i = 1:N-1
                    if(IsBreak)
                        break;
                    end
        ...
        function obj = DDP(CostFun_,StateFun_)
